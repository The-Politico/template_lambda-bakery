...
import { INVALID_TEMPLATE_TYPE } from 'Bakery/server/errors';
...
export default async function(templateName, payload, filepath = '') {
  ...
  await writeFile(path.join(BAKERY_TMP_DIR, filepath, 'index.html'), html);
  ...
  if (staticPath) {
    glob.sync(`${staticPath}/**`).filter(fp => {
      ...
      const filename = fp.split('/')[fp.split('/').length - 1];
      cp.sync(fp, path.join(BAKERY_TMP_DIR, filepath, filename));
    });
  }
};
